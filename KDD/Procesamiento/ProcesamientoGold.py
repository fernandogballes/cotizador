...
def procces_gold_poblacion_activa_ocupada():
    df_activa = pd.read_excel(paths.SILVER_POBLACION_ACTIVA_PATH)
    df_ocupada = pd.read_excel(paths.SILVER_POBLACION_OCUPADA_PATH)
...
def process_gold_atr():
    df = pd.read_excel(paths.SILVER_ATR_2001_2022_PATH)
...
def process_gold_accidentes_trafico():
    df = pd.read_excel(paths.SILVER_ACCIDENTES_TRAFICO_PATH)
...
def create_gold():
    ...
    Funciones.create_excel(df_gold, paths.GOLD_DATA_PATH)
